/* =============================================
   SaaSERP Victoria — JS Compartido
...
const ICONS = {
  ...
  store:     `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>`,
  ...
  trash2:    `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>`,
  ...
  car:       `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 17H3v-7l2-4h14l2 4v7h-2"/><circle cx="7.5" cy="17.5" r="2.5"/><circle cx="16.5" cy="17.5" r="2.5"/></svg>`,
  ...
  calendar:  `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>`,
  archive:   `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="21 8 21 21 3 21 3 8"/><rect x="1" y="3" width="22" height="5"/><line x1="10" y1="12" x2="14" y2="12"/></svg>`,
  ...
  messageCircle:`<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>`,
  ...
  bookUser:  `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/><circle cx="12" cy="9" r="2"/><path d="M9 15a3 3 0 0 1 6 0"/></svg>`,
  ...
  xCircle:   `<svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>`,
  ...
  briefcase: `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="14" rx="2" ry="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></svg>`,
};
...
function getUser() {
    ...
    if (!d) return null;
    return {
        id:        d['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier'] || d.sub || '',
        email:     d['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress'] || d.email || '',
        rol:       d['http://schemas.microsoft.com/ws/2008/06/identity/claims/role'] || d.role || '',
        negocioId: d.NegocioId ? parseInt(d.NegocioId) : null,
        nombre:    d.name || d.email || '',
    };
}

function logout() {
    localStorage.removeItem(TOKEN_KEY);
    window.location.href = '/web/WebAuth/Login';
}

function requireAuth(redirectTo) {
    const user = getUser();
    if (!user) { window.location.href = '/web/WebAuth/Login'; return null; }
    return user;
}
...
function renderUserInfo(user, theme = 'admin') {
    const avatarClass = theme === 'negocio' ? 'user-avatar user-avatar-negocio' : 'user-avatar';
    const roleClass   = theme === 'negocio' ? 'user-role user-role-negocio' : 'user-role';
    return `
      <div class="user-info">
        <div class="${avatarClass}">${user.email.charAt(0).toUpperCase()}</div>
        <div style="overflow:hidden">
          <div class="user-email truncate">${user.email}</div>
          <div class="${roleClass}">${user.rol}</div>
        </div>
      </div>
      <button class="btn-logout" onclick="logout()">${ICONS.logOut} Cerrar Sesión</button>
    `;
}
